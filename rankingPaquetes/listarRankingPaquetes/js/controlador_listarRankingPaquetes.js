...
function mostrarListas()
{
    ...
    {     
        if(listarSucursales[i][0].toLowerCase().includes(sfiltro))
        {
            ...
            let sCategoria = fila.insertCell(); // INSERTA EN LA FILA EN LA POS 0 EL NOMBRE
            let sCantidad = fila.insertCell();
            // let sTelefono = fila.insertCell();
            // let sModificar = fila.insertCell();
            // let sDesactivar = fila.insertCell();           

            sCategoria.appendChild(document.createTextNode(listarSucursales[i][0]));
            sCantidad.appendChild(document.createTextNode(listarSucursales[i][1]));
