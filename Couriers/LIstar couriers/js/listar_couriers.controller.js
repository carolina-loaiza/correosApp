...
function mostrarCouriers() {
    ...
    for(let i = 0; i < listaCouriers.length; i++) {
        if (listaCouriers[i].includes(sFiltro)) {
            ...
            cNombre.appendChild(document.createTextNode(listaCouriers[i]));
            ...
            atag.dataset.nombre = listaCouriers[i];
            ...
            botonDesactivar.dataset.nombre = listaCouriers[i];
